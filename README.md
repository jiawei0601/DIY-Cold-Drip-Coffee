...
```
┌────────────────────────────────────────────────┐
│  Eneloop Pro AA x4 (4.8V~5.8V)                │
│       │                                         │
│       ├──── MT3608 升壓 ──→ 12V ──→ D4184 ──→ Kamoer NKP 幫浦
│       │                              │
│       ├──── DC-DC 降壓 ──→ 5V ──→ ESP32 CYD (USB 供電)
│       │
│       └──── 分壓電阻 ────→ GPIO35 (ADC 電壓監測)
│              100kΩ + 10kΩ
│
│  ESP32 CYD GPIO 分配：
│       GPIO27 ──→ D4184 MOSFET 訊號腳 (幫浦控制)
│       GPIO35 ──→ 分壓電阻中間點 (電壓監測, ADC1_CH7)
└────────────────────────────────────────────────┘
```
